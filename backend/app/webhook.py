...
async def reply_topic_menu(reply_token: str, topic_key: str, db, user, today_date, tomorrow_date, today_override, tomorrow_override) -> None:
    ...
    if topic_key == "topic_time":
        await reply_with_quick_reply(
            reply_token,
            "時間設定：可用一條引導式流程設定身份、住家、目的地、固定到達時間與提醒星期，也可臨時調整今天或明天。",
    ...
    try:
        for event in events:
            ...
            if isinstance(event, FollowEvent):
                set_pending_field(db, user.id, "identity_type")
                reply_token = event.reply_token
                await reply_with_quick_reply(
                    reply_token,
                    "歡迎使用智慧通勤助理！👋\n"
                    + field_prompt_for_profile("identity_type", get_profile(db, user.id)),
            ...
            if isinstance(event, PostbackEvent):
                ...
                if postback_action in {"template_toggle_weekday", "template_preset", "template_save"}:
                    ...
                    if postback_action == "template_preset":
                        ...
                        await reply_schedule_template_weekday_picker(
                            reply_token,
                            pending["time"],
                            pending["label"],
                            pending["days"],
                            "請確認這組排程適用的星期，選完後請按「儲存排程」。",
                        )
                        continue
