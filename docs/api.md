...
**Not mounted** 9 further routes in `dashboard.py` and 1 in `liff_routes.py` — see
...
`/dashboard/*`. It is registered after the route decorators above, and Starlette matches in
registration order, so `GET /dashboard` and `GET /dashboard/family` resolve to the handlers rather
than to the mount.
...
Only three postback actions exist. Almost all interaction is text-driven — a broad but shallow LINE
...
**Source** `backend/app/` @ `e10e6d9` — route inventory generated by
