...
class Phase1StabilityTests(unittest.TestCase):
    ...
    def test_dashboard_routes_and_shared_schedule_summary_are_wired(self):
        ...
        self.assertIn("def dashboard_commute_target_schedule", main_py)
        self.assertIn("active_schedules_for_date(schedules, now_dt.date())", main_py)
        self.assertIn("setInterval(refresh, refreshMs)", dashboard_view_py)
        self.assertIn("setInterval(render, 1000)", dashboard_view_py)
        self.assertIn("commute.departureDateTime", dashboard_view_py)
        self.assertIn("commute.departureDate || commute.targetDate", dashboard_view_py)
