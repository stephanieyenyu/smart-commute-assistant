...
class Phase5DashboardTests(unittest.TestCase):
    ...
    def test_reminder_settings_reply_has_toggle_quick_replies(self):
        ...
        self.assertIn("可用下方按鈕調整", webhook_py)
    ...
    def test_line_webhook_has_schedule_controls(self):
        ...
        self.assertIn('"label": "平日"', webhook_py)
        self.assertIn('"label": "假日"', webhook_py)
        self.assertIn('"text": "排程假日"', webhook_py)
        self.assertIn("preset_row(preset_buttons[:2])", webhook_py)
        self.assertIn("preset_row(preset_buttons[2:])", webhook_py)
        ...
        self.assertIn("weekday_save", webhook_py)
        self.assertIn("save_weekday_schedule", webhook_py)
        self.assertIn("儲存排程", webhook_py)
        self.assertIn("暫停排程", webhook_py)
        self.assertNotIn("已更新星期，選好後請按", webhook_py)
        self.assertNotIn("切換週", webhook_py)
        self.assertNotIn("已設定提醒日", webhook_py)
    ...
    def test_schedule_repeat_picker_uses_native_line_ui(self):
        ...
        self.assertIn("action=weekday_save", webhook_py)
    ...
    def test_household_management_and_computer_kiosk_guides_are_wired(self):
        ...
        self.assertIn('"remove_household_member"', webhook_py)
        self.assertIn('"confirm_remove_household_member"', webhook_py)
        self.assertIn('"label": "移除成員"', webhook_py)
        self.assertIn("format_remove_member_prompt", webhook_py)
        self.assertIn("remove_member_quick_replies", webhook_py)
        ...
        self.assertIn("remove_user_from_household", crud_py)
        ...
        self.assertIn("實體電腦 Dashboard 操作方式", webhook_py)
    ...
    def test_rich_menu_topics_and_prompt_coverage_are_wired(self):
        ...
        self.assertIn('"看板管理"', webhook_py)
        self.assertNotIn('"看板家庭"', webhook_py)
    ...
    def test_identity_labels_and_multi_schedule_are_wired(self):
        ...
        self.assertIn("WIZARD_WEEKDAY_QUICK_REPLIES", webhook_py)
        self.assertIn("WIZARD_DESTINATION_PENDING_FIELDS", webhook_py)
        self.assertIn("wizard_office_location", webhook_py)
        self.assertIn("學校名稱", webhook_py)
        self.assertIn("wizard_weekday_prompt", webhook_py)
        ...
        self.assertEqual(schedule.arrival_label("學校"), "到校時間")
